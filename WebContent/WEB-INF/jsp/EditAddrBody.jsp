...
<bean:define id="addr" name="customerMgr" property="selectedAddr" type="com.konakart.appif.AddressIf"/>
...
<html:form action="/EditAddrSubmit.do" styleId="form1">  
	<%
		com.konakart.forms.RegisterCustomerForm myForm = (com.konakart.forms.RegisterCustomerForm) request.getAttribute("RegisterCustomerForm"); 
		if (myForm != null){
			myForm.setFirstName((myForm.getFirstName()==null)?addr.getFirstName():myForm.getFirstName());
			myForm.setLastName((myForm.getLastName()==null)?addr.getLastName():myForm.getLastName());
			myForm.setCompany((myForm.getCompany()==null)?addr.getCompany():myForm.getCompany());
			myForm.setStreetAddress((myForm.getStreetAddress()==null)?addr.getStreetAddress():myForm.getStreetAddress());
			myForm.setStreetAddress1((myForm.getStreetAddress1()==null)?addr.getStreetAddress1():myForm.getStreetAddress1());
			myForm.setSuburb((myForm.getSuburb()==null)?addr.getSuburb():myForm.getSuburb());
			myForm.setPostcode((myForm.getPostcode()==null)?addr.getPostcode():myForm.getPostcode());
			myForm.setCity((myForm.getCity()==null)?addr.getCity():myForm.getCity());
			myForm.setState((myForm.getState()==null)?addr.getState():myForm.getState());
			myForm.setCountryId((myForm.getCountryId()==0)?addr.getCountryId():myForm.getCountryId());
			myForm.setAddrTelephone((myForm.getAddrTelephone()==null)?addr.getTelephoneNumber():myForm.getAddrTelephone());
			myForm.setAddrTelephone1((myForm.getAddrTelephone1()==null)?addr.getTelephoneNumber1():myForm.getAddrTelephone1());
			myForm.setAddrEmail((myForm.getAddrEmail()==null)?addr.getEmailAddr():myForm.getAddrEmail());
		}				
	%>

	<div class="body">
		<div class="body-header">
			<img style="float:right" src="<%=kkEng.getImageBase()%>/table_background_address_book.gif" border="0" alt="<bean:message key="edit.addr.body.title"/>" title=" <bean:message key="edit.addr.body.title"/> " width="<%=kkEng.getHeadingImageWidth()%>" height="<%=kkEng.getHeadingImageHeight()%>">
			<bean:message key="edit.addr.body.title"/>
		 </div>
		<div class="body-content-div">
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="2" class="body-content-tab">
				<tr>
					<td ><b><bean:message key="edit.addr.body.newaddrbook"/></b></td>
			 ...
			 <div class="msg-box">
				...
				<%if (kkEng.getConfig("ACCOUNT_GENDER").equalsIgnoreCase("TRUE") ) { %>
					<tr>
							...
							<%custForm.setGender(addr.getGender());%>
				...
				<logic:equal name="addr"  property="isPrimary" value="false">	
					<tr>
						<td colspan="2"><img src="<%=kkEng.getImageBase()%>/pixel_trans.gif" border="0" alt="" width="100%" height="10"></td>
					</tr>
					<tr>
						<td colspan="2" ><html:checkbox name="RegisterCustomerForm" property="setAsPrimaryBool"/> <bean:message key="edit.addr.body.setasprimary"/>.</td>
					</tr>	
				</logic:equal>										
			</table>
			</div>
	...
	<div class="tile">
			<div class="button-left"><div class="button-right"><div class="button-bottom"><div class="button-bottom-left"><div class="button-bottom-right"><div class="button-top"><div class="button-top-left"><div class="button-top-right">
				<div class="button-tile">
					<a style="float:right" onclick="javascript:document.forms['form1'].submit();" class="button"><span><bean:message key="common.update"/></span></a>
	...
	<!-- Need to ensure that it contains a date formatted with the correct template so that we don't get validation errors -->
